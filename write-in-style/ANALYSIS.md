...
agent behavior to `custom_llm_agent.our_custom_agent` and uses local
`app_utils.py` helpers for shared UI/login/model behavior. The private `myfunc`
dependency has been removed and those helpers are vendored in `app_utils.py`.
...
- Several experiments reference external services, private indexes, and local
  database/search credentials.
...
- Replaced `requirements.txt` with the direct dependencies used by the archived
  scripts, removed the private `myfunc` Git dependency, and vendored its helper
  surface in `app_utils.py`.
